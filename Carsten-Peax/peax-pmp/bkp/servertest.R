...
output[[curchoose_mir]] <- renderUI({
  ...
  curmirPlot<-paste0("mirPlot1",curtr)
  output[[curmirPlot]] <- renderPlot({
     message("DEBUG: render hist Plot")
      hist_data<-getPheno1()
      hist_data<-getPhenoExp(hist_data,1)
      bp<-hist_data
      bp<-bp[bp$Group==1,]
      bp<-bp[input[["hist_input"]]]
     if (length(bp)>0) {
       hist(bp[,1],main=paste0("Distribution of ",input[["hist_input"]]),xlab=input[["hist_input"]],axes=TRUE,right=FALSE)
     }
   },width=300,height=250) # this is the actual size of the plot output

  curmirPlot<-paste0("mirPlot2",curtr)
  output[[curmirPlot]] <- renderPlot({
      message("DEBUG: render hist Plot")
      hist_data<-getPheno1()
      hist_data<-getPhenoExp(hist_data,1)
  ...
  curmirPlot<-paste0("mirPlot3",curtr)
  output[[curmirPlot]] <- renderPlot({
      message("DEBUG: render hist Plot")
      hist_data<-getPheno1()
      hist_data<-getPhenoExp(hist_data,1)
  ...
  curmirPlot<-paste0("mirPlot4",curtr)
  output[[curmirPlot]] <- renderPlot({
      message("DEBUG: render hist Plot")
      hist_data<-getPheno1()
      hist_data<-getPhenoExp(hist_data,1)
  ...
  output[[curmirPlot]] <- renderPlot({
      ...
      bp<-hist_data
      bp<-bp[bp$Group==1,]
      bp<-bp[input[["hist_input2"]]]
      if (length(bp)>0) {
          hist(bp[,1],main=paste0("Distribution of ",input[["hist_input2"]]),xlab=input[["hist_input2"]],axes=TRUE,right=FALSE)
      }
  },width=300,height=250) # this is the actual size of the plot output

  curmirPlot<-paste0("Plot2",curtr)
  output[[curmirPlot]] <- renderPlot({
      message("DEBUG: render hist Plot")
      hist_data<-getPheno1()
      hist_data<-getPhenoExp(hist_data,1)
